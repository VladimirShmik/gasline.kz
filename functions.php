...
function register_my_menus() {
    register_nav_menus(
        array(
            'header-menu' => __( 'Главное меню (Главная)' ),
            'mobile-menu' => __( 'Мобильное меню' ),
            'footer-menu' => __( 'Футер меню' ),
            'category-menu' => __( 'Меню категорий' ),
            'header-sub-menu' => __( 'Главное меню (Внутренние)' ),
        )
    );
};
...
// Отключаем хук woocommerce_after_shop_loop_item
remove_action('woocommerce_after_shop_loop_item', 'woocommerce_template_loop_add_to_cart', 10);

// Переделка кнопок корзины
function custom_woocommerce_template_loop_add_to_cart() {
    ...
    echo '<a href="' . esc_url($product->add_to_cart_url()) . '" data-quantity="1" class="button product_type_simple add_to_cart_button ajax_add_to_cart product-box__add-cart">' . esc_html__('В корзину', 'woocommerce') . '</a>';

    echo '<a href="' . esc_url($product_link) . '" class="button product_type_simple product-box__more">' . esc_html__('Подробнее', 'woocommerce') . '</a>';
...
//Заменяем хук цены в единичном товаре
function custom_single_product_price() {
    echo '<p class="price__title">Цена: </p>';
